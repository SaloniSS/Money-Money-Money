...
const AlternatePrice = (props) => {
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(["price"], function (result) {
      if (!result) setUnitPrice("Set initial.");
  ...
  return (
    <div className="content">
      ...
      <p>
        <strong>Price of one {props.mode}: </strong> ${unitPrice}
      </p>
      <p>
        <strong>Current Price: </strong> ${price}
      </p>
      <p>
        <strong>This is equivalent to: </strong> {newPrice} {props.mode}
      </p>
      <Content />
    </div>
  );
